...
//    struct accept_awaitable_ {
//        explicit accept_awaitable(iouring &uring, std::shared_ptr<sockaddr_storage_t> address) :
//        client_addr_(std::move(address)) ,server_socket_{uring.server_socket_} {
//            sqe_ = io_uring_get_sqe(&uring.ring_);
//            auto ptr = client_addr_.get();
//            io_uring_prep_accept(sqe_, server_socket_, (struct sockaddr *) ptr, &client_addr_len_, 0);
//
//        }
...
//                std::shared_ptr<sockaddr_storage_t> address_;
//                explicit awaiter(io_uring_sqe *sqe, std::shared_ptr<sockaddr_storage_t>& address) : entry{sqe}, address_(address)  {}
...
//                    return {.fd_ = req_data_.client_fd_,
//                            .ep_ = create_endpoint(address_.get())};
//                }
//            };
//
//            return awaiter {sqe_, client_addr_};
//        }
//    private:
//        io_uring_sqe *sqe_;
////        std::shared_ptr<sockaddr_storage_t> client_addr_;
//        sockaddr_storage_t client_addr_;
